...
gemini_rate_limiter = RateLimiter(rate=60, per=60)  # 60 requests per minute


@gemini_rate_limiter
def _get_gemini_client() -> genai.Client:
    """Get or create Gemini client using configured API key (rate-limited)."""
...
async def _wait_for_operation(
    ...
    """Wait for a long-running operation to complete using configured polling settings (with retry)."""
    ...
    if not operation.done:
        raise TimeoutError(
            f"Operation timed out after {max_attempts * poll_interval}s. "
            "Try increasing poll_interval_seconds or max_poll_attempts."
        )
